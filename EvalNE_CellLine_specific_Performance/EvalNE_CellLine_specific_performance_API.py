import os
os.environ["OPENBLAS_NUM_THREADS"] = "38"
...
assert os.getcwd().split('/')[-1] == "EvalNE_CellLine_specific_Performance", "Wrong working directory"
...
def eval_other(nee, scoresheet, npr_ppi, npr_dep, save_preds=None, save_embs=True, emb_size=128, ppi_scaffold="",
               screening="", pos_thresh=None):
    """
...
all_file_loc = glob.glob(f"{BASE_PATH}/LP_train_test_splits{screening}/"
                         f"{ppi_scaffold}_split_nprPPI{args.npr_ppi}_nprDEP{args.npr_dep}{pos_thresh_str}/"
                         f"{disease.replace(' ','_')}/*")

if not os.path.isdir(f"{BASE_PATH}/CellLine_Specific_Benchmark_Res{screening}/"
                     f"{ppi_scaffold}/{disease.replace(' ','_')}"):
    os.makedirs(f"{BASE_PATH}/CellLine_Specific_Benchmark_Res{screening}/"
                f"{ppi_scaffold}/{disease.replace(' ','_')}")
...
for repeat in range(3):
    ...
    assert np.array_equal(np.hstack((np.ones(train_edges[np.where(train_labels == 1)[0]].shape[0]),
                                     np.zeros(train_edges[np.where(train_labels == 0)[0]].shape[0]))),
                          traintest_split.train_labels)
    ...
    # Eval others
    eval_other(nee, scoresheet, save_preds=None, save_embs=True, emb_size=args.emb_dim, ppi_scaffold=ppi_scaffold,
               npr_ppi=args.npr_ppi, npr_dep=args.npr_dep, screening=args.screening, pos_thresh=pos_thresh_str)
...
scoresheet.write_all(f"{BASE_PATH}/"
                     ...
                     f"5epochs02valShuffled_nprPPI{args.npr_ppi}_nprDEP{args.npr_dep}"
                     f"_cellLinePerformance_emb{args.emb_dim}_trained80percent.txt",
                     repeats='all')

scoresheet.write_pickle(f"{BASE_PATH}/"
                        f"CellLine_Specific_Benchmark_Res{screening}/{ppi_scaffold}/{disease.replace(' ','_')}/"
                        f"5epochs02valShuffled_nprPPI{args.npr_ppi}_nprDEP{args.npr_dep}"
                        f"_cellLinePerformance_emb{args.emb_dim}_trained80percent.pickle")
