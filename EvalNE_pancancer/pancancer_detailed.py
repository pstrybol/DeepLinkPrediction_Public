...
for repeat in range(3):
    ...
    for dis in diseases:
        ...
        # --------------------------------- TEST SET ----------------------------------------------------------------
        test_edges = pd.DataFrame(read_h5py(list(filter(re.compile(rf'(.*/test_all_cls_{repeat}.hdf5)').match,
        ...
        test_sets_per_disease[dis] = tuple((test_edges.values, test_labels))
    ...
    # TESTING
    test_total_edges = np.vstack([i[0] for i in test_sets_per_disease.values()])
    test_total_labels = np.hstack([i[1] for i in test_sets_per_disease.values()])

    # np.savetxt(f"test_labels_rep{repeat}.txt", test_total_labels, fmt="%i")

    nee = construct_EvalNE_splits(pan_cancer_train_edges, pan_cancer_train_labels, pan_cancer_val_edges,
    ...
    eval_other(nee=nee, scoresheet=scoresheet, npr_ppi=npr_ppi, npr_dep=npr_dep, disease="Pan Cancer",
               BASE_PATH=BASE_PATH,
               save_preds=f"{BASE_PATH}/EvalNE_pancancer/method_predictions_{ppi_scaffold}{screening}", save_embs=True,
               emb_size=emb_dim, ppi_scaffold=ppi_scaffold, screening=screening, pos_thresh='', train_ratio=80)

    eval_baselines(nee=nee, directed=False, scoresheet=scoresheet,
                   save_preds=f"{BASE_PATH}/EvalNE_pancancer/method_predictions_{ppi_scaffold}{screening}")

    # evaluate_dw(ppi_scaffold=ppi_scaffold, disease="Pan Cancer", screening=screening, train_edges=pan_cancer_train_edges,
    #             train_labels=pan_cancer_train_labels, test_edges=test_total_edges,
    ...
    #             save_preds=f"{BASE_PATH}/EvalNE_pancancer/method_predictions_{ppi_scaffold}{screening}", train_ratio=80)
    ...
    dl_obj = main_DLP.main(tr_e=pan_cancer_train_edges, tr_e_labels=pan_cancer_train_labels, inputgraph=pan_nw_obj,
                           merge_method='weighted-l2', predifined_embs=f"deepwalk-opene_Pan_Cancer_{ppi_scaffold}_"
                                                                    f"embsize128_80percent{screening}" \
    ...
    pd.to_pickle(tmp_df, f"{BASE_PATH}/EvalNE_pancancer/method_predictions_{ppi_scaffold}{screening}/"
                         f"PanCancer_revised_setting_DLP-DeepWalk_{ppi_scaffold}_{repeat}{screening}.pickle")

# scoresheet.write_all(f"{BASE_PATH}/CellLine_Specific_Benchmark_Res{screening}/{ppi_scaffold}/Pan_Cancer/"
#                      f"PanCancer_revised_setting.txt",
#                      repeats='all')
#
# scoresheet.write_pickle(f"{BASE_PATH}/CellLine_Specific_Benchmark_Res{screening}/{ppi_scaffold}/Pan_Cancer/"
#                         f"PanCancer_revised_setting.pickle")
# -------------------------------------------------------- READ IN RESULTS -------------------------------------------
