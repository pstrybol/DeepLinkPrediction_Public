...
assert os.getcwd().split('/')[-1] == "EvalNE_PPI_Performance", "Wrong working directory"
...
def eval_other(nee, scoresheet, save_preds=None, save_embs=False, emb_size=128, ppi_scaffold="", screening=""):
    ...
    for i in range(0, len(methods)):
        ...
        save_embs = f"{methods[i]}_{disease.replace(' ', '_')}_{ppi_scaffold}_embsize{emb_size}_80percent{screening}"\
...
print(ppi_scaffold, disease, pos_thresh_str, '\n')
all_file_loc = glob.glob(f"{BASE_PATH}/"
                         f"LP_train_test_splits{screening}/"
...
for repeat in range(3):
    print('PPI performance - Repetition {} of experiment - TRAINED ON 80%'.format(repeat))
    ...
    test_labels = read_h5py(list(filter(re.compile(rf'(.*/labeltest_all_ppis_{repeat}.hdf5)').match, all_file_loc))[0])
    ...
    # Eval others
    eval_other(nee, scoresheet, save_preds=None, save_embs=False, emb_size=args.emb_dim)
...
scoresheet.write_all(f"{BASE_PATH}/"
                     ...
                     f"{disease.replace(' ','_')}_complete_metricPerofrmance_emb{args.emb_dim}.txt", repeats='all')

scoresheet.write_pickle(f"{BASE_PATH}/"
                        f"PPI_Benchmark_Res{screening}/"
                        f"{ppi_scaffold}/{disease.replace(' ','_')}/"
                        f"5epochs02valShuffled_nprPPI{args.npr_ppi}_nprDEP{args.npr_dep}_"
                        f"{disease.replace(' ','_')}_complete_metricPerofrmance_emb{args.emb_dim}.pickle")
