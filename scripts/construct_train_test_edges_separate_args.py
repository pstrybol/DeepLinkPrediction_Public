...
# screening = ''
dis_df = pd.read_csv(f"{BASE_PATH}/depmap_specific_cancer_df/{ppi_scaffold}_{disease.replace(' ','_')}{screening}.csv",
...
def train_test_separate(npr_ppi, npr_dep, train_ratio, val_ratio, negative_dep_threshold,
                        positive_dep_threshold, gene2int, out_path, split_id):
    ...
    assert not set(heterogeneous_network_obj.nodes) - set(np.unique(X_train[np.where(y_train == 1)].ravel())),\
        "ERROR NDOES LOST DURING SPLIT"
...
# This is for computational speed up
backend = 'multiprocessing'
path_func = delayed(train_test_separate)
Parallel(n_jobs=num_splits, verbose=True, backend=backend)(
path_func(npr_ppi, npr_dep, train_ratio, val_ratio, negative_dep_threshold, positive_dep_threshold,
          gene2int, out_path, split) for split in range(num_splits))
